  ...
  useEffect(() => {
    async function fetchData() {
      const [movieResponse, creditsResponse, recommendationsResponse] = await Promise.all([
        ...
        fetch(`https://api.themoviedb.org/3/movie/${params.id}/credits`, options),
        fetch(`https://api.themoviedb.org/3/movie/${params.id}/recommendations`, options),
      ]);
  ...
  return (
    <div className="p-[2rem] lg:px-[10rem] 2xl:px-[14rem]">
      <h1 className="text-[1.5rem] max-w-[13.75rem] md:max-w-[30rem] text-[black] font-semibold">
        {loading ? <Skeleton width={200} style={{ borderRadius: "8px" }} /> : data.title}
      </h1>
      ...
      <div className="flex justify-between items-center mt-4">
        <p>
          {loading ? (
            ...
            `${data.release_date.replaceAll("-", ".")} · ${data.adult ? "PG" : "U"} · ${minuteConverter(data.runtime)}`
          )}
        </p>
        <div className="flex items-center gap-2">
          ...
          <div>
            {loading ? (
              <Skeleton width={50} height={20} style={{ borderRadius: "8px" }} />
            ) : (
              <>
      ...
      <div className="flex gap-4 justify-between items-center mt-[2rem]">
        ...
        <div>
          <div className="grid gap-2 grid-cols-2 md:grid-cols-3 lg:grid-cols-5">
            {loading
              ? Array.from({ length: 3 }).map((_, i) => (
                  <Skeleton key={i} width={50} height={20} style={{ borderRadius: "8px" }} />
                ))
          ...
          <p className="mt-[1rem] lg:mt-[3rem]">
            {loading ? <Skeleton count={3} style={{ borderRadius: "8px" }} /> : data.overview}
          </p>
        </div>
      </div>
